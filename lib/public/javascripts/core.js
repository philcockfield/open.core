...
(function(/*! Stitch !*/) {
  ...
  */
  ...
  var Base, Model, excludeMethod, excludeMethods;
  ...
  module.exports = Model = (function() {
    ...
    function Model(params) {
      ...
      for (key in model) {
        ...
        if (excludeMethod(key)) {
          continue;
        }
        this[key] = model[key];
      }
      _.extend(fnFetch, Backbone.Events);
      fnFetch.onComplete = function(handler) {
        return fnFetch.bind('complete', handler);
      };
    }
    ...
    Model.prototype.fetch = function(options) {
      ...
      return model.fetch({
        success: function(m, res) {
          return onComplete(res, true, false, options.success);
        },
        error: function(m, res) {
          return onComplete(res, false, true, options.error);
        }
      });
    };
    return Model;
